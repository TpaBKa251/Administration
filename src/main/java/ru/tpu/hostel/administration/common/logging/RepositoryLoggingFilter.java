...
import static ru.tpu.hostel.administration.common.logging.Message.FINISH_REPOSITORY_METHOD_EXECUTION;
import static ru.tpu.hostel.administration.common.logging.Message.REPOSITORY_METHOD_EXECUTION_EXCEPTION;
import static ru.tpu.hostel.administration.common.logging.Message.START_REPOSITORY_METHOD_EXECUTION;
...
public class RepositoryLoggingFilter {
    ...
    @Around("execution(public * ru.tpu.hostel.administration.repository.*Repository*.*(..))")
    public Object logRepositoryMethod(ProceedingJoinPoint joinPoint) throws Throwable {
        ...
        String repositoryName = Arrays.stream(joinPoint.getTarget().getClass().getInterfaces())
                .filter(i -> i.getPackageName().startsWith("ru.tpu.hostel.administration.repository"))
                .findFirst()
